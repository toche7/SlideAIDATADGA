...
paginate: true
size: 16:9
footer: "AI for Data | Session 02 | Data Exploration with Google Colab"
math: katex
---
...
# Session 02
# Data Exploration with Google Colab
...
จัดเป็นหมวดตาม workflow เพื่อเลือกใช้ prompt ได้ง่ายขึ้น
...
---
## คำศัพท์ที่ควรรู้: ตอนโหลดข้อมูล
...
---
## คำศัพท์ที่ควรรู้: ตอน clean ข้อมูล
...
| Data standardization | การปรับสเกลข้อมูลให้อยู่ในมาตรฐานเดียวกัน (เช่น ค่าเฉลี่ยเป็น 0 และส่วนเบี่ยงเบนมาตรฐานเป็น 1) | "ช่วยเขียนโค้ด Python เพื่อทำ standardization ให้คอลัมน์ `age` และ `income` ด้วยวิธี z-score และเปรียบเทียบก่อน-หลังการแปลง" |
| Uniqueness | การตรวจสอบว่าค่าในคอลัมน์ซ้ำกันมากน้อยเพียงใด | "ช่วยเขียนโค้ด Python เพื่อตรวจสอบ uniqueness ของทุกคอลัมน์ด้วย `nunique()` และสรุปผลเป็นตาราง" |
| Unique values | ค่าที่ไม่ซ้ำกันในแต่ละคอลัมน์ | "ช่วยเขียนโค้ด Python เพื่อแสดง unique values ของคอลัมน์ `region` และ `area` พร้อมจำนวนค่า" |
...
| คำศัพท์ | ความหมาย | ตัวอย่างการใช้ใน prompt |
|---|---|---|
| Cardinality | จำนวนค่าที่แตกต่างกันในคอลัมน์หนึ่งๆ | "ช่วยเขียนโค้ด Python เพื่อตรวจสอบ cardinality ของทุกคอลัมน์ และระบุคอลัมน์ที่มี cardinality สูงผิดปกติ" |
...
| Boxplot | กราฟที่ใช้ดูการกระจายของข้อมูล ค่ามัธยฐาน และค่าผิดปกติ | "ช่วยเขียนโค้ด Python เพื่อสร้าง boxplot ของคอลัมน์ `value` แยกตาม `region`" |
| Scatter plot | กราฟจุดที่ใช้ดูความสัมพันธ์ระหว่างตัวแปรเชิงตัวเลขสองตัว | "ช่วยเขียนโค้ด Python เพื่อสร้าง scatter plot ระหว่าง `age` และ `waiting_time` พร้อมเส้นแนวโน้ม" |
| Histogram | กราฟแท่งแสดงการกระจายความถี่ของค่าตัวแปรเชิงตัวเลข | "ช่วยเขียนโค้ด Python เพื่อสร้าง histogram ของคอลัมน์ `value` และอธิบายการกระจายข้อมูล" |
| Bar chart | กราฟแท่งเปรียบเทียบค่าระหว่างหมวดหมู่ | "ช่วยเขียนโค้ด Python เพื่อสร้าง bar chart เปรียบเทียบค่าเฉลี่ย `value` ตาม `level_of_edu`" |


---
## คำศัพท์ที่ควรรู้: ตอนสรุปผล
